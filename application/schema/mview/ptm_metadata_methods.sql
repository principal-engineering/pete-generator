create materialized view ptm_metadata_methods as
select case
       ...
       nvl2((select 1
              from ptm_metadata_function_result a
 ...
 where not (procedure_name is null and object_type = 'PACKAGE') -- package spec
   and not exists (select *
          from ptm_metadata_mtds_wth_rec_arg m
         where case
...
create unique index ptm_metadata_methods_pk on
  ptm_metadata_methods(package_name, method_name, subprogram_id)
;
